...
const ProjectCard = ({gen, setGenerations, forCommunity = false} : { gen: Project, setGenerations: React.Dispatch<React.SetStateAction<Project[]>>, forCommunity?: boolean}) => {
  ...
  const togglePublish = async (projectId: string) => {
    ...
    <div key = {gen.id} className=" mb-4 break-inside-avoid">
        ...
        {/* preview */}
        <div className= {`${gen?.aspectRatio === '9:16' ? 'aspect-9/16' : gen?.aspectRatio === '1:1' ? 'aspect-square' : 'aspect-video' } relative overflow-hidden`} >
            {gen.generatedImage && (
            ...
            {!forCommunity && (
                ...
                onMouseLeave={()=> {setMenuOpen(false)}}
                 className="absolute right-3 top-3 sm:opacity-0 group-hover:opacity-100 transition fkex items-center gap-2">
                   <div className="absolute top-3 right-3">
                    ...
                    <div className="flex flex-col items-end w-32 text-sm">
                        <ul className={`text-xs ${menuOpen ? 'block' : 'hidden'} verflow-hidden right-0 peer-focus:block hover:block w-40 bg-black/50 backdrop-blur 
        ...
        <div className="p-4">
            <div className="flex items-start justify-between gap-4">
                <div className="flex-1">
                    ...
                    <p className="text-sm text-gray-400">Created: {gen.createdAt ? new Date(gen.createdAt).toLocaleDateString() : 'N/A'}</p>
                    {gen.updatedAt && (
            ...
            {!forCommunity && (
                <div className="mt-4 grid grid-cols-2 gap-3">
                    <GhostButton className="text-xs justify-center" onClick={()=>{navigate(`/results/${gen.id}`); scrollTo(0,0)}}>
                        View Details
                    </GhostButton>
